...
| `testify` | Assertions (`assert`, `require`) and test suites; reduces boilerplate. |
| `gomock` / `mockery` | Generate mocks from interfaces; use for event bus, ports, and other dependencies. |
| `go-mock` (uber) | Hand-written mocks with a small API; no codegen. |

**Summary:** Use for the majority of regression coverage. Prefer testing Logic via events (emit intents, assert state) rather than full engine bootstrap. Keep tests next to source (`*_test.go`).
...
| Builds confidence that the system works end-to-end for a slice of behavior.           | Failures can be harder to diagnose (which component is wrong?).     |
